...
const BenSurplus = () => {
  ...
  const [radius, setRadius] = useState<number>(() => {
    const savedRadius =
      localStorage.getItem("search_radius") ||
      localStorage.getItem("app_radius");
  ...
  const loadDonations = async () => {
    try {
      ...
      let response: any;
      if (currentLocation) {
        response = await getDonationsNearby(
          currentLocation.lat,
          currentLocation.lng,
          savedRadius || 10,
        );
  ...
  const filteredDonations = donations.filter((donation) => {
    ...
    const matchesWilaya =
      ...
      donation.pickupAddress
        ?.toLowerCase()
        .includes(selectedWilaya.toLowerCase());
  ...
  const handleConfirmReserve = async () => {
    ...
    if (quantity > selectedDonation.totalQuantity) {
      toast.error(
        `Maximum quantity is ${selectedDonation.totalQuantity} ${selectedDonation.unit}`,
      );
      return;
    }
        ...
        <div className="p-6">
          <div className="mb-6">
            ...
            <p className="text-gray-500 mt-1">
              Browse and reserve food near you within {radius} km
            </p>
          </div>
          ...
          {error && (
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm">
              {error}
              <button onClick={loadDonations} className="ml-3 underline">
                Try again
              </button>
            </div>
          )}
              ...
              {filteredDonations.map((donation) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${getExpiryColor(donation.expirationDate)}`}
                    >
                  ...
                  <p className="text-primary-600 text-sm mt-1 flex items-center gap-1">
                    ...
                    {donation.donor?.user?.name || "Donor"}
                  </p>
                  <div className="flex flex-wrap gap-3 text-sm text-gray-500 mt-3">
                    <span className="flex items-center gap-1">
                      <FaBox className="text-xs" /> {donation.totalQuantity}{" "}
                      {donation.unit}
                    </span>
                    <span className="flex items-center gap-1">
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6">
            ...
            <div className="mb-4 space-y-2">
              <p className="text-gray-600">
                <strong className="text-gray-800">Food:</strong>{" "}
                {selectedDonation.foodType}
              </p>
              <p className="text-gray-600">
                <strong className="text-gray-800">Donor:</strong>{" "}
                {selectedDonation.donor?.user?.name}
              </p>
              <p className="text-gray-600">
                <strong className="text-gray-800">Available:</strong>{" "}
                {selectedDonation.totalQuantity} {selectedDonation.unit}
              </p>
              <p className="text-gray-600">
                <strong className="text-gray-800">Pickup:</strong>{" "}
                {selectedDonation.pickupAddress}
              </p>
              {selectedDonation.pickupTime && (
                <p className="text-gray-600">
                  <strong className="text-gray-800">Pickup Time:</strong>{" "}
                  {selectedDonation.pickupTime}
                </p>
              )}
            </div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Quantity ({selectedDonation.unit})
              </label>
              <input
                type="number"
                min="1"
